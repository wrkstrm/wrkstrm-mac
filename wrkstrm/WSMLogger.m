 ...
 * WSMLogger = ContextLogger (Black/White) + DDTTYLogger + Multiple customer Formatters
 *
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark - NSLog Macro Defines
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark - Mac and iOS Color Compatibility macros - Copied from DDTTYLogger
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

/**
 * Some simple defines to make life easier on ourself and ensure Mac & iOS compatibility!
 */
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark - WSMLogFormatter: Custom Log Styles
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

NSString *const kWSMLogFormatKeyQueue = @"kWSFormatKeyQueue";
NSString *const kWSMLogFormatKeyFile = @"kWSFormatKeyFile";
NSString *const kWSMLogFormatKeyFunction = @"kWSFormatKeyFunction";

@implementation WSMLogFormatter {
...
- (id)init {
    if ((self = [super init])) {
        ...
        self.formatStyle = kWSMLogFormatStyleNSLog;
...
- (void)setFormatStyle:(WSMLogFormatStyle)logStyle {
    //NSLogError(@"We get called: %i", logStyle);
    if (_formatStyle != logStyle) {
        _formatStyle = logStyle;
        
        switch (logStyle) {
            case kWSMLogFormatStyleCBL: {
                dateFormatString = @"HH:mm:ss.SSS";
            }break;
            case kWSMLogFormatStyleQueue: {
                dateFormatString = @"mm:ss.SSS";
            }break;
            default: {
        ...
        if (loggerCount <= 1) { // Single-threaded mode.
            [threadUnsafeDateFormatter setDateFormat:dateFormatString];
        } else {  // Multi-threaded mode.
            NSString *key = @"DispatchQueueLogFormatter_NSDateFormatter";
            [[NSThread currentThread].threadDictionary[key] setDateFormat:dateFormatString];
        }
    }
}
...
+ (BOOL)isValidLogFormatKey:(NSString *)key {
    return [key isEqualToString:kWSMLogFormatKeyFile] ||
    [key isEqualToString:kWSMLogFormatKeyFunction] ||
    [key isEqualToString:kWSMLogFormatKeyQueue];
}
...
- (NSString *)stringFromDate:(NSDate *)date {
    ...
    if (loggerCount <= 1) {
        ...
        stringFromDate = [threadUnsafeDateFormatter stringFromDate:date];
    } else {
        // Multi-threaded mode.
...
- (NSMutableDictionary *)styleAtrributeLengthDictionary {
    return !_styleAtrributeLengthDictionary ?  (_styleAtrributeLengthDictionary =  [@{} mutableCopy]) : _styleAtrributeLengthDictionary;
}

- (void)restrictKey:(NSString *)key valueArray:(NSArray *)values {
    NSAssert(values, @"The restriction values array is nil.");
    NSAssert(values.count == 2, @"The restriction values array does not contain 2 values");
    NSAssert([WSMLogFormatter isValidLogFormatKey:key], @"An invalid WSMLogFormaKey was specified.");
    
    NSMutableDictionary *dictionaryForStyle = [@{ key: values } mutableCopy];
    
    [self setDictionaryForStyle:dictionaryForStyle];
}

- (void)setMinLengthForKey:(NSString *)key value:(NSUInteger)value {
    NSAssert([WSMLogFormatter isValidLogFormatKey:key], @"An invalid WSMLogFormaKey was specified.");
    NSArray *restrictValues = self.styleAtrributeLengthDictionary[@(self.formatStyle)][key];
    NSLogWarn(@"RestrictValues: %@", restrictValues);
    restrictValues = !restrictValues ? @[@(value), @0] : @[@(value),restrictValues[1]];
    NSMutableDictionary *dictionaryForStyle = [@{ key: restrictValues } mutableCopy];
    
    [self setDictionaryForStyle:dictionaryForStyle];
}

- (void)setMaxLengthForKey:(NSString *)key value:(NSUInteger)value {
    NSAssert([WSMLogFormatter isValidLogFormatKey:key], @"An invalid WSMLogFormaKey was specified.");
    NSArray *restrictValues = self.styleAtrributeLengthDictionary[@(self.formatStyle)][key];
    NSLogWarn(@"RestrictValues: %@", restrictValues);
    restrictValues = !restrictValues ? @[@0, @(value)] : @[restrictValues[0], @(value)];
    NSMutableDictionary *dictionaryForStyle = [@{ key: restrictValues } mutableCopy];
...
- (void)setDictionaryForStyle:(NSMutableDictionary *)styleDictionary  {
    OSSpinLockLock(&lock); {
        if (!self.styleAtrributeLengthDictionary[@(self.formatStyle)]) {
            self.styleAtrributeLengthDictionary[@(self.formatStyle)] = styleDictionary;
        } else {
            [self.styleAtrributeLengthDictionary[@(self.formatStyle)] addEntriesFromDictionary:styleDictionary];
        }
    } OSSpinLockUnlock(&lock);
    //NSLogWarn(@"Stored Values: %@", self.styleAtrributeLengthDictionary[@(self.formatStyle)]);
}

- (NSString *)formatString:(NSString *)key attribute:(NSString *)attribute {
    NSMutableDictionary *dictionaryForStyle = self.styleAtrributeLengthDictionary[@(self.formatStyle)];
    NSUInteger minQueueLength = [dictionaryForStyle[key][0] unsignedIntegerValue];
    NSUInteger maxQueueLength = [dictionaryForStyle[key][1] unsignedIntegerValue];
    
    NSString *formattedAttribute;
    NSUInteger labelLength = [attribute length];
    ...
    if ((maxQueueLength > 0) && (labelLength > maxQueueLength)) {
        // Truncate
        formattedAttribute = [attribute substringToIndex:maxQueueLength];
        ...
        formattedAttribute = [NSString stringWithFormat:@"%@%s", attribute, spaces];
    } else {
...
- (NSString *)queueThreadLabelForLogMessage:(DDLogMessage *)logMessage {
    ...
    if (logMessage->queueLabel) {
        ...
        NSInteger length = sizeof(names) / sizeof(char *);
    ...
    if (useQueueLabel || useThreadName)  {
        ...
        fullLabel = useQueueLabel ? @(logMessage->queueLabel) : logMessage->threadName;
        
        OSSpinLockLock(&lock); {
            abrvLabel = _replacements[fullLabel];
        } OSSpinLockUnlock(&lock);
        
        queueThreadLabel = abrvLabel ? abrvLabel : fullLabel;
    } else {
        queueThreadLabel = [NSString stringWithFormat:@"%x", logMessage->machThreadID];
    }
    return [self formatString: kWSMLogFormatKeyQueue attribute: queueThreadLabel];
}

- (NSString *)formatLogMessage:(DDLogMessage *)logMessage {
    NSString *formattedLogString;
    NSString *logFile = [@(logMessage->file) lastPathComponent];
    logFile = [logFile substringToIndex:logFile.length - 2];
    logFile = [self formatString:kWSMLogFormatKeyFile
                       attribute:logFile];
    
    NSString *logFunction = @(logMessage->function);
    logFunction = [self formatString:kWSMLogFormatKeyFunction
                           attribute:logFunction];
    
    NSString *logTimestamp = [self stringFromDate:(logMessage->timestamp)];
    
    NSString *logQueue = [self queueThreadLabelForLogMessage:logMessage];
    
    switch (self.formatStyle) {
        case kWSMLogFormatStyleCBL: {
            NSString *timestampTrailer = [logQueue isEqualToString:@"main"] ? @"|" : @"‖";
            formattedLogString = [NSString stringWithFormat: @"%@%@ %@",
                                  logTimestamp, timestampTrailer, logMessage->logMsg];
        } break;
        case kWSMLogFormatStyleQueue: {
            formattedLogString = [NSString stringWithFormat: @"%@-%@[%@|%@] %@",
                                  logTimestamp, logQueue, logFile, logFunction,
                                  logMessage->logMsg];
        } break;
        case kWSMLogFormatStyleDefault: {
            formattedLogString = [NSString stringWithFormat:@"%@ [%@] %@", logTimestamp, logQueue, logMessage->logMsg];
        } break;
        default: {
            formattedLogString = logMessage->logMsg;
        } break;
    }
...
#pragma mark - WSMLogger: Make Logger changes here (Blacklist / Whitelist)

@interface WSTTYLoggerColorProfile : NSObject {
@public
    NSInteger mask;
    NSInteger context;
...
- (id)initWithForegroundColor:(OSColor *)fgColor backgroundColor:(OSColor *)bgColor flag:(NSInteger)mask context:(NSInteger)ctxt;
...
+ (void)initialize  {
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (term && (strcasestr(term, "color") != NULL))  {
            isaColorTTY = YES;
        ...
        NSLogInfo(@"WSTTYLogger: isaColorTTY = %@", (isaColorTTY ? @"YES" : @"NO"));
        NSLogInfo(@"WSTTYLogger: isaColor256TTY: %@", (isaColor256TTY ? @"YES" : @"NO"));
        NSLogInfo(@"WSTTYLogger: isaXcodeColorTTY: %@", (isaXcodeColorTTY ? @"YES" : @"NO"));
    });
}
...
+ (void)passiveLoad {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        WSMLogger *sLogger = [WSMLogger sharedInstance];
        [DDLog removeLogger:sLogger];
        [DDLog addLogger:sLogger];
        NSLogWarn(@"QuickStart: WSMLogger was loaded to DDLog.");
    });
}
...
- (id)init {
    if ((self = [super init])) {
        ...
        WSMLogFormatter *newformatter = [WSMLogFormatter new];
        newformatter.formatStyle = kWSMLogFormatStyleNSLog;
        [self setLogFormatter:newformatter];
        ...
        processID = [NSString stringWithFormat:@"%li", (long)getpid()];
...
- (WSMLogFormatStyle)formatStyle {
    return [formatter isKindOfClass:WSMLogFormatter.class] ? [(WSMLogFormatter *)formatter formatStyle] : (WSMLogFormatStyle) nil;
}

- (void)setFormatStyle:(WSMLogFormatStyle)style {
    [self accessDirectlyAndInQueue:^{
        WSMLogFormatStyle logStyle = style;
        //NSLogWarn(@"Style: %i", logStyle);
        
        if ([self->formatter isKindOfClass:WSMLogFormatter.class]) {
            [(WSMLogFormatter *) self->formatter setFormatStyle:logStyle];
        }
    }];
}
...
- (id)objectForKeyedSubscript:(id)key {
    ...
    if ([formatter isKindOfClass: [WSMLogFormatter class]]) {
        OSSpinLockLock(&lock); {
            WSMLogFormatter *wsFormatter = (WSMLogFormatter *) formatter;
            array = wsFormatter.styleAtrributeLengthDictionary[key];
        } OSSpinLockUnlock(&lock);
    }
    return array;
}
...
- (void)setObject:(id)obj forKeyedSubscript:(id)key {
    [self accessDirectlyAndInQueue:^{
        if ([obj isKindOfClass: [NSArray class]]) {
            [(WSMLogFormatter *) self->formatter setDictionaryForStyle: [@{key: obj} mutableCopy]];
        } else if ([obj isKindOfClass: [NSNumber class]]) {
            [(WSMLogFormatter *) self->formatter setDictionaryForStyle: [@{key: @[obj, obj]} mutableCopy]];
        } else {
            NSAssert(NO, @"Only Arrays or Numbers are accepted.");
        }
    }];
}

- (void)accessDirectlyAndInQueue:(void (^)(void))block {
...
- (void)setContextStyle:(WSMLogContextStyle)ctxStyle {
    OSSpinLockLock(&lock); {
        _contextStyle = ctxStyle;
    } OSSpinLockUnlock(&lock);
}

- (BOOL)shouldLog:(NSInteger)loggingContext {
    switch (_contextStyle) {
        case kWSMLogContextStyleBlack:
            return ![self isOnList:loggingContext];
        case kWSMLogContextStyleWhite:
            return [self isOnList:loggingContext];
        default: return YES;
    }
}
...
- (NSMutableSet *)whiteList {
    return !_whiteList ? (_whiteList = [NSMutableSet new]) : _whiteList;
}

- (NSMutableSet *)blackList {
    return !_blackList ? (_blackList = [NSMutableSet new]) : _blackList;
}

- (NSArray *)currentList {
    NSArray *result;
    NSMutableSet *currentContextSet = [self currentContextSet];
    OSSpinLockLock(&lock); {
        result = [currentContextSet allObjects];
    } OSSpinLockUnlock(&lock);
    return result;
}
...
- (void)addToList:(NSInteger)loggingContext {
    NSMutableSet *currentContextSet = [self currentContextSet];
    OSSpinLockLock(&lock); {
        [currentContextSet addObject:@(loggingContext)];
    } OSSpinLockUnlock(&lock);
}
...
- (void)removeFromList:(NSInteger)loggingContext {
    NSMutableSet *currentContextSet = [self currentContextSet];
    OSSpinLockLock(&lock); {
        [currentContextSet removeObject:@(loggingContext)];
    } OSSpinLockUnlock(&lock);
}
...
- (BOOL)isOnList:(NSInteger)loggingContext {
    ...
    OSSpinLockLock(&lock); {
        result = [[self currentContextSet] containsObject:@(loggingContext)];
    } OSSpinLockUnlock(&lock);
    return result;
}

- (NSMutableSet *)currentContextSet {
    switch (_contextStyle) {
        case kWSMLogContextStyleBlack: {
        ...
        default: {
            NSLog(@"Invalid / No Context Style Specified.");
            return nil;
        } break;
    }
}
...
- (void)setForegroundColor:(OSColor *)txtColor backgroundColor:(OSColor *)bgColor forFlag:(NSInteger)mask {
    [self setForegroundColor:txtColor backgroundColor:bgColor forFlag:mask context:0];
}

- (void)setForegroundColor:(OSColor *)txtColor backgroundColor:(OSColor *)bgColor forFlag:(NSInteger)mask context:(NSInteger)ctxt {
    dispatch_block_t block = ^{ @autoreleasepool {
        
        WSTTYLoggerColorProfile *newColorProfile =
        [[WSTTYLoggerColorProfile alloc] initWithForegroundColor:txtColor
                                                 backgroundColor:bgColor
                                                            flag:mask
                                                         context:ctxt];
        
        NSLogInfo(@"WSTTYLogger: newColorProfile: %@", newColorProfile);
        
        NSUInteger i = 0;
        
        for (WSTTYLoggerColorProfile *colorProfile in self->colorProfilesArray) {
        ...
        if (i < [self->colorProfilesArray count]) {
            self->colorProfilesArray[i] = newColorProfile;
        } else {
...
- (void)setForegroundColor:(OSColor *)txtColor backgroundColor:(OSColor *)bgColor forTag:(id <NSCopying>)tag {
    ...
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        self->colorProfilesDict[tag] = newColorProfile;
    }};
...
- (void)logMessage:(DDLogMessage *)logMessage {
    if (![self shouldLog:logMessage->logContext]) return;
    ...
    if (logMsg) {
        ...
        WSTTYLoggerColorProfile *colorProfile = nil;
        ...
        if (colorsEnabled) {
            ...
            if (colorProfile == nil) {
                for (WSTTYLoggerColorProfile *cp in colorProfilesArray) {
        ...
        if (isFormatted) {
            ...
            NSInteger milliseconds = (NSInteger)((epoch - floor(epoch)) * 1000);
            
            char ts[24];
            len = snprintf(ts, 24, "%04ld-%02ld-%02ld %02ld:%02ld:%02ld:%03ld", // yyyy-MM-dd HH:mm:ss:SSS
...
+ (void)initialize_colors_256 {
    ...
    NSInteger ri; // r increment
    NSInteger gi; // g increment
    NSInteger bi; // b increment
...
+ (NSUInteger)codeIndexForColor:(OSColor *)inColor {
    ...
    for (OSColor *color in colors) {
        ...
        if (distance < lowestDistance) {
            ...
            NSLogVerbose(@"WSTTYLogger: New best index = %lu", (unsigned long)bestIndex);
        }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
@implementation WSTTYLoggerColorProfile

- (id)initWithForegroundColor:(OSColor *)fgColor backgroundColor:(OSColor *)bgColor flag:(NSInteger)aMask context:(NSInteger)ctxt {
    if ((self = [super init])) {
        mask = aMask;
        context = ctxt;
        ...
        if (fgColor && isaColorTTY) {
            ...
            fgCodeIndex = [WSMLogger codeIndexForColor:fgColor];
        ...
        } else { // No foreground color or no color support
        ...
        } else { // No background color or no color support
...
- (NSString *)description {
    return [NSString stringWithFormat:
            @"<WSMLoggerColorProfile: %p mask:%li ctxt:%li fg:%u,%u,%u bg:%u,%u,%u fgCode:%@ bgCode:%@>",
            self, (long)mask, (long)context, fg_r, fg_g, fg_b, bg_r, bg_g, bg_b, fgCodeRaw, bgCodeRaw];
}
